// generated with ast extension for cup
// version 0.8
// 2/9/2025 9:20:17


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(FirstConst FirstConst) { }
    public void visit(ActParsOpt ActParsOpt) { }
    public void visit(StatementList StatementList) { }
    public void visit(ExprAddopList ExprAddopList) { }
    public void visit(MethodStart MethodStart) { }
    public void visit(Factor Factor) { }
    public void visit(ConstList ConstList) { }
    public void visit(DeclList DeclList) { }
    public void visit(Designator Designator) { }
    public void visit(StatementListVar StatementListVar) { }
    public void visit(Term Term) { }
    public void visit(VarAssignList VarAssignList) { }
    public void visit(FormParsList FormParsList) { }
    public void visit(FormParsOptional FormParsOptional) { }
    public void visit(MulOp MulOp) { }
    public void visit(RelOp RelOp) { }
    public void visit(VarDecList VarDecList) { }
    public void visit(ExprList ExprList) { }
    public void visit(Expr Expr) { }
    public void visit(ExprOption ExprOption) { }
    public void visit(AddOp AddOp) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(ExprStartOpt ExprStartOpt) { }
    public void visit(Decl Decl) { }
    public void visit(ElseOption ElseOption) { }
    public void visit(Statement Statement) { }
    public void visit(VarDecl VarDecl) { }
    public void visit(VarAssign VarAssign) { }
    public void visit(CondTermORList CondTermORList) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(Param Param) { }
    public void visit(RelopExprOption RelopExprOption) { }
    public void visit(CondFactANDList CondFactANDList) { }
    public void visit(Label Label) { visit(); }
    public void visit(Percent Percent) { visit(); }
    public void visit(DivOp DivOp) { visit(); }
    public void visit(MulopOp MulopOp) { visit(); }
    public void visit(LessThenEqual LessThenEqual) { visit(); }
    public void visit(LessThen LessThen) { visit(); }
    public void visit(GreaterEqual GreaterEqual) { visit(); }
    public void visit(Greater Greater) { visit(); }
    public void visit(NonEquals NonEquals) { visit(); }
    public void visit(EqualsEquals EqualsEquals) { visit(); }
    public void visit(ClassFactor ClassFactor) { visit(); }
    public void visit(ArrayFactor ArrayFactor) { visit(); }
    public void visit(DesignatorFuncCall DesignatorFuncCall) { visit(); }
    public void visit(DesignatorVar DesignatorVar) { visit(); }
    public void visit(ExprFactor ExprFactor) { visit(); }
    public void visit(BoolFactor BoolFactor) { visit(); }
    public void visit(CharFactor CharFactor) { visit(); }
    public void visit(NumFactor NumFactor) { visit(); }
    public void visit(TermMulop TermMulop) { visit(); }
    public void visit(TermFactor TermFactor) { visit(); }
    public void visit(NoRelationOp NoRelationOp) { visit(); }
    public void visit(RelationOpWithExpression RelationOpWithExpression) { visit(); }
    public void visit(CondFact CondFact) { visit(); }
    public void visit(NoAndConditionList NoAndConditionList) { visit(); }
    public void visit(AndConditionList AndConditionList) { visit(); }
    public void visit(CondTerm CondTerm) { visit(); }
    public void visit(NoExpressionList NoExpressionList) { visit(); }
    public void visit(ExpressionList ExpressionList) { visit(); }
    public void visit(ActPars ActPars) { visit(); }
    public void visit(SetOp SetOp) { visit(); }
    public void visit(AssignOp AssignOp) { visit(); }
    public void visit(DesignClass DesignClass) { visit(); }
    public void visit(DesignArr DesignArr) { visit(); }
    public void visit(DesignIdent DesignIdent) { visit(); }
    public void visit(NoActualParameter NoActualParameter) { visit(); }
    public void visit(ActualParameterList ActualParameterList) { visit(); }
    public void visit(ThreeDesignList ThreeDesignList) { visit(); }
    public void visit(DesignAssign DesignAssign) { visit(); }
    public void visit(DesignDecrement DesignDecrement) { visit(); }
    public void visit(DesignIncrement DesignIncrement) { visit(); }
    public void visit(DesignFunctionCalling DesignFunctionCalling) { visit(); }
    public void visit(DesignAssignExpr DesignAssignExpr) { visit(); }
    public void visit(NoExprAddopList NoExprAddopList) { visit(); }
    public void visit(ExprAddopListGlobal ExprAddopListGlobal) { visit(); }
    public void visit(Minus Minus) { visit(); }
    public void visit(Plus Plus) { visit(); }
    public void visit(ExprStartWithMinus ExprStartWithMinus) { visit(); }
    public void visit(ExprStartWithoutMinus ExprStartWithoutMinus) { visit(); }
    public void visit(ExprDesignator ExprDesignator) { visit(); }
    public void visit(ExprFirstOpt ExprFirstOpt) { visit(); }
    public void visit(NoOrConditionList NoOrConditionList) { visit(); }
    public void visit(OrConditionList OrConditionList) { visit(); }
    public void visit(Condition Condition) { visit(); }
    public void visit(NoStatements NoStatements) { visit(); }
    public void visit(StatmntList StatmntList) { visit(); }
    public void visit(NoExprStmt NoExprStmt) { visit(); }
    public void visit(ExprOptionStmt ExprOptionStmt) { visit(); }
    public void visit(NoElseStmt NoElseStmt) { visit(); }
    public void visit(ElseOptionStmt ElseOptionStmt) { visit(); }
    public void visit(ErrorStmt ErrorStmt) { visit(); }
    public void visit(DoWhileStmtWithDesignatorStmt DoWhileStmtWithDesignatorStmt) { visit(); }
    public void visit(DoWhileStmtWithCondition DoWhileStmtWithCondition) { visit(); }
    public void visit(DoWhileStmt DoWhileStmt) { visit(); }
    public void visit(StmtList StmtList) { visit(); }
    public void visit(PrintStmt1 PrintStmt1) { visit(); }
    public void visit(PrintStmt PrintStmt) { visit(); }
    public void visit(ReadStmt ReadStmt) { visit(); }
    public void visit(ReturnStmt ReturnStmt) { visit(); }
    public void visit(ContinueStmt ContinueStmt) { visit(); }
    public void visit(BreakStmt BreakStmt) { visit(); }
    public void visit(IfElseStmt IfElseStmt) { visit(); }
    public void visit(DesignatorStmt DesignatorStmt) { visit(); }
    public void visit(SingleParam SingleParam) { visit(); }
    public void visit(FormalParsList FormalParsList) { visit(); }
    public void visit(Param_array Param_array) { visit(); }
    public void visit(Param_var Param_var) { visit(); }
    public void visit(FormPars FormPars) { visit(); }
    public void visit(NoStatementList NoStatementList) { visit(); }
    public void visit(StatementListGlobal StatementListGlobal) { visit(); }
    public void visit(NoVarDecList NoVarDecList) { visit(); }
    public void visit(VariableDeclList VariableDeclList) { visit(); }
    public void visit(MethodDecl MethodDecl) { visit(); }
    public void visit(NoFormParsOptional NoFormParsOptional) { visit(); }
    public void visit(FormalParsOptional FormalParsOptional) { visit(); }
    public void visit(MethodStartVoid MethodStartVoid) { visit(); }
    public void visit(MethodStartType MethodStartType) { visit(); }
    public void visit(MethodSignature MethodSignature) { visit(); }
    public void visit(SingleVarAssign SingleVarAssign) { visit(); }
    public void visit(RestVariableDecl RestVariableDecl) { visit(); }
    public void visit(VarAssign_array VarAssign_array) { visit(); }
    public void visit(VarAssign_var VarAssign_var) { visit(); }
    public void visit(VarDeclError VarDeclError) { visit(); }
    public void visit(VariableDecl VariableDecl) { visit(); }
    public void visit(Type Type) { visit(); }
    public void visit(ListElems ListElems) { visit(); }
    public void visit(SingleConstAssign SingleConstAssign) { visit(); }
    public void visit(ConstantList ConstantList) { visit(); }
    public void visit(ConstBool ConstBool) { visit(); }
    public void visit(ConstChar ConstChar) { visit(); }
    public void visit(ConstNum ConstNum) { visit(); }
    public void visit(ConstDecl ConstDecl) { visit(); }
    public void visit(NoMethodDeclarations NoMethodDeclarations) { visit(); }
    public void visit(MethodDeclarations MethodDeclarations) { visit(); }
    public void visit(VarDeclaration VarDeclaration) { visit(); }
    public void visit(ConstDeclaration ConstDeclaration) { visit(); }
    public void visit(NoDeclarations NoDeclarations) { visit(); }
    public void visit(Declarations Declarations) { visit(); }
    public void visit(ProgName ProgName) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
