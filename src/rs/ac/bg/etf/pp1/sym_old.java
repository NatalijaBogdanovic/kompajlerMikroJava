package rs.ac.bg.etf.pp1;

public class sym_old {
	//kljucne reci
	public static final int PROG=1;
	public static final int BREAK=2;
	public static final int CLASS=3;
	public static final int ELSE=4;
	public static final int CONST=5;
	public static final int IF=6;
	public static final int NEW=7;
	public static final int PRINT=8;
	public static final int READ=9;
	public static final int RETURN=10;
	public static final int VOID=11;
	public static final int EXTENDS=12;
	public static final int CONTINUE=13;
	public static final int UNION=14;
	public static final int DO=15;
	public static final int WHILE=16;
	
	public static final int MAP=17;
	public static final int INTERFACE=18;
	
	//vrste tokena
	public static final int IDENT=19;
	public static final int NUM_CONST=20;
	public static final int CHAR_CONST=21;
	public static final int BOOL_CONST=22;
	
	//operatori
	public static final int PLUS = 23;
	public static final int MINUS = 24;
	public static final int MUL = 25;
	public static final int DIV = 26;
	public static final int PERCENT = 27;
	public static final int EQUALS_EQUALS = 28;
	public static final int NOT_EQUALS = 29;
	public static final int GT = 30;
	public static final int GTE = 31;
	public static final int LT = 32;
	public static final int LTE = 33;
	public static final int AND = 34;
	public static final int OR = 35;
	public static final int EQUALS = 36;
	public static final int INCREMENT = 37;
	public static final int DECREMENT = 38;
	public static final int SEMICOLON = 39;
	public static final int COLON = 40;
	public static final int COMMA = 41;
	public static final int DOT = 42;
	public static final int LEFT_ROUND_PARENTHESES = 43;
	public static final int RIGHT_ROUND_PARENTHESES = 44;
	public static final int LEFT_SQUARE_PARENTHESES = 45;
	public static final int RIGHT_SQUARE_PARENTHESES = 46;
	public static final int LEFT_WIGGLY_PARENTHESES = 47;
	public static final int RIGHT_WIGGLY_PARENTHESES = 48;
	
	public static final int EOF = 49;

}
